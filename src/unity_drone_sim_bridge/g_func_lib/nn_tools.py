...
from unity_drone_sim_bridge.g_func_lib.load_database import load_surrogate_database, generate_surrogate_synthetic_data
from unity_drone_sim_bridge.g_func_lib.nn_models import *
...
def LoadNN(hidden_size, hidden_layer, test_size=0.2, synthetic=False, rt=False, gpu=True, naive=False, use_yolo=True):
    EXPERIMENT_NAME = "SurrogateNetworkFixedOutput_hs64_hl2_lr0_01_e1000_bs1_ts0_2_synTrue"
    ...
    model = SurrogateNetworkFixedOutput(True, 64, 2)
    ...
    l4c_model = l4c.realtime.realtime_l4casadi.RealTimeL4CasADi(model, approximation_order=2, device="cuda" if gpu else "cpu") \
        if rt \
        else l4c.L4CasADi(model, model_expects_batch_dim=True, device="cuda" if gpu else "cpu")
    return l4c_model
...
def run_training_experiments(experiments: List[Dict[str, Any]]) -> None:
    ...
    for exp in experiments:
        ...
        if exp['synthetic']:
            synthetic_X, synthetic_Y = generate_surrogate_synthetic_data(X, int(0.3 * len(X)))
...
def random_input_test(casadi_quad_approx_func, l4c_model_order2, gpu,):
                            ...
                            for batch_size in batch_sizes:
                                for hidden_size in hidden_sizes:
                                    for hidden_layer in hidden_layers:
                                        exp = {
                                            'name': f'{model}_hs{hidden_size}_hl{hidden_layer}_lr{lr}_e{epochs}_bs{batch_size}_ts{test_size}_syn{synthetic}'.replace(".", "_"),
                                            'model': model,
                                            'synthetic': synthetic,
                                            'use_yolo': use_yolo,
                                            'test_size': test_size,
