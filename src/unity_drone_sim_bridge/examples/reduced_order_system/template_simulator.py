...
class Simulator:
    ...
    def update_nearest_trees(self):
        ...
        self.ReducedOrderIdxsLambda = n_nearest_objects(self.x_k['Xrobot'][:2].reshape(1, -1), self.trees_pos, num=self.dim_lambda)
        self.ResidualIdxsLambda = np.setdiff1d(np.arange(self.trees_pos.shape[0]), self.ReducedOrderIdxsLambda)
        self.ReducedOrderIdxsTree = n_nearest_objects(self.x_k['Xrobot'][:2].reshape(1, -1), self.trees_pos, num=self.dim_obs)
