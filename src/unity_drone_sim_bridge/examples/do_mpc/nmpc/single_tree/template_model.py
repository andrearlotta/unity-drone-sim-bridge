...
def template_model(symvar_type='MX', g=None, rt=False):
    """
    --------------------------------------------------------------------------
    ...
    lambda_ = model.set_variable(var_type='_x', var_name='lambda', shape=1)
    lambda_prev = model.set_variable(var_type='_x', var_name='lambda_prev', shape=1)
    model.set_variable(var_type='_x', var_name='y', shape=1)
    
    # Input struct (optimization variables):
    u_x_robot = model.set_variable(var_type='_u', var_name='x_robot_set', shape=3)

    # Time-varying parameters
    x_tree_lambda = np.array([[0.0,0.0]])
    x_tree_obs = np.array([[0.0,0.0]])
    mapped_g = g_map_casadi(g,x_tree_lambda)
