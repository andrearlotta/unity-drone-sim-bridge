...
def template_model(x_trees, symvar_type='MX', g=None, rt=False):
    """
    --------------------------------------------------------------------------
    ...
    lambda_ = model.set_variable(var_type='_x', var_name='lambda', shape=x_trees.shape[0])
    lambda_prev = model.set_variable(var_type='_x', var_name='lambda_prev', shape=x_trees.shape[0])
    model.set_variable(var_type='_x', var_name='y', shape=x_trees.shape[0])
    
    # Input struct (optimization variables):
    u_x_robot = model.set_variable(var_type='_u', var_name='x_robot_set', shape=3)

    mapped_g = g_map_casadi(g, x_trees)
    ...
    model.set_expression('y', mapped_g(x_robot,x_trees))
    model.set_expression('cost_function',  -(H(lambda_) -  H(lambda_prev)))
