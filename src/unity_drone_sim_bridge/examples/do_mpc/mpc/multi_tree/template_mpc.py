...
def template_mpc(model, T, N, silence_solver = False):
    """
    --------------------------------------------------------------------------
    ...
    #mpc.set_rterm(x_robot_set=np.array(3*[1e-8]))

    mpc.bounds['lower','_u','x_robot_set'] = 2 * [-.5] + [-np.pi/4]
    mpc.bounds['upper','_u','x_robot_set'] = 2 * [+.5] + [+np.pi/4]
